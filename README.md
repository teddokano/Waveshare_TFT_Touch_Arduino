...
`SDBitmapViewerDemo`|The same idea taken further, on **FRDM-MCXA153 and FRDM-MCXN947 only**: tap/swipe/long-press navigation, a screen saver, directional wipes between pictures, and an SD-card log -- all configured from a single JSON file on the card. See [its own README](examples/SDBitmapViewerDemo/README.md); it stops with an `#error` on other boards, so the portable `SDBitmapViewer` above stays the one to start from
