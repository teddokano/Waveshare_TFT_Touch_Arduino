...
void setup(void)
{
	...
	// are confirmed correct for this rotation on real hardware: the raw
	// X channel tracks the panel's vertical axis correctly (top=low,
	// bottom=high), while the raw Y channel tracks the horizontal axis
	// but reversed (left=high, right=low) -- hence swapXY with only
	// invertX set. Run the TouchCalibration example if your panel needs
	// a tighter raw range than the full 0..4095 span.
